...
fn main() {
    ...
    let buffer = ImageBuffer::from_fn(width, height, |x, y| {
        ...
        let rho = 2.0;
        let xfloat = rho * theta.cos() * phi.cos();
        let zfloat = rho * theta.sin() * phi.cos();
        let yfloat = rho * phi.cos();
        let ftotal = noise_ref.get([xfloat, yfloat, zfloat]);
        blend_pixel(ftotal, light, dark)
    });
